...
function populateTopicSelector(){
  ...
  (topicsBySubject[SUBJECT]||[]).forEach(topic=>{
    ...
    fetch(csvPath,{method:'HEAD'}).then(res=>{
      if(!res.ok) option.textContent+=" (未設定)";
    }).catch(()=>{
...
function checkAnswer(selected){
  ...
  if(selected===q.answer){ correctSound.play(); score++; showConfetti(); }
  else { wrongSound.play(); mistakes.push({q:q.question, correct:q.options[q.answer], explanation:q.explanation}); }
  setTimeout(()=>{
    currentQuestion++;
