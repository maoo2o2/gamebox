let quizData=[];let currentQuestion=0;let score=0;let mistakes=[];
...
function populateTopicSelector(){
  ...
  (topicsBySubject[SUBJECT]||[]).forEach(topic=>{
    ...
    fetch(csvPath,{method:'HEAD'}).then(res=>{ if(!res.ok) option.textContent+=" (未設定)"; })
...
function loadSelectedQuiz(){
  ...
  fetch(path).then(res=>{ if(!res.ok) throw new Error(path); return res.text();})
  .then(csv=>{ quizData=parseCSV(csv); startBtn.disabled=true; quizArea.classList.remove("hidden");
    resultContainer.classList.add("hidden"); currentQuestion=0;score=0;mistakes=[]; showQuestion(); })
...
function parseCSV(csv){
  ...
  for(let i=1;i<lines.length;i++){
    ...
    if((hasImage&&vals.length>=8)||(!hasImage&&vals.length>=7)){
      let idx=0;const q={question:vals[idx++]};
...
function showQuestion(){
  ...
  quizContainer.innerHTML=`<div class="question"><h2>問題${currentQuestion+1}</h2>${imgHtml}<p>${q.question}</p><div class="options">${q.options.map((opt,i)=><button onclick="checkAnswer(${i})">${opt}</button>).join("")}</div></div>`;
}
function checkAnswer(selected){
  const q=quizData[currentQuestion];
  document.querySelectorAll(".options button").forEach((b,i)=>{
    b.disabled=true; if(i===q.answer)b.classList.add("correct"); if(i===selected&&i!==q.answer)b.classList.add("wrong");
  });
  if(selected===q.answer){correctSound.play();score++;showConfetti();} else{wrongSound.play();mistakes.push({q:q.question,correct:q.options[q.answer],explanation:q.explanation});}
  setTimeout(()=>{currentQuestion++; if(currentQuestion<quizData.length)showQuestion(); else showResult();},1200);
}
function showResult(){
  resultSound.play();
  ...
  mistakesContainer.innerHTML=mistakes.length?("<h3>間違えた問題：</h3>"+mistakes.map(m=>`<p><strong>Q:</strong>${m.q}<br><strong>正解:</strong>${m.correct}<br><em>${m.explanation}</em></p>`).join("")):"<p>全問正解！すばらしい！</p>";
}
function restartQuiz(){currentQuestion=0;score=0;mistakes=[];resultContainer.classList.add("hidden");quizContainer.classList.remove("hidden");showQuestion();}
